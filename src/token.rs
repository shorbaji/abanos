...
const GOOGLE_CLIENT_ID: &str =
    "169688466353-b2g33gq8h5k0b73b5h155lggoarcaphs.apps.googleusercontent.com";
...
fn login() -> Result<String, String> {
    ...
    login_with_browser().or_else(|_| login_without_browser())
}
...
fn login_with_browser() -> Result<String, String> {
    ...
    let server = Server::new("localhost:6871", move |request| {
...
use openidconnect::{
    AuthenticationFlow, ClientId, CsrfToken, IssuerUrl, Nonce, RedirectUrl, Scope,
};
...
fn get_authorization_url(redirect_url: Option<String>) -> String {
    ...
    let client = if let Some(redirect_url) = redirect_url {
        CoreClient::from_provider_metadata(provider_metadata, google_client_id, None)
