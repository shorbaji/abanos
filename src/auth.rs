...
fn get_credential_with_oauth2(args: &crate::Args) -> Result<Credential, String> {
    ...
    let client_id = env::var("ABANOS_AUTH_CLIENT_ID")
        .ok()
        .unwrap_or("".to_string());
    let client_secret = env::var("ABANOS_AUTH_CLIENT_SECRET")
        .ok()
        .unwrap_or("".to_string());
    ...
    let credential = rx
        ...
        .and_then(|credential| {
            sender
                .send(()) // we send a stop message to the server
                .map(|_| credential)
